(ns tosmonitor.store
  ...
  This repo archives exactly ONE company (東京電力ホールディングス株式会社 /
  Tokyo Electric Power Company Holdings, LEI 5299004EMJ3R4RVR5Y75) -- unlike
  `commitledger.store`'s multi-application directory, this Store holds a
  SINGLE `:company` record and a SINGLE `:baseline` record (the latest
  archived ToS snapshot this actor treats as ground truth to compare
  candidates against), plus the append-only audit ledger and
  change-proposal draft history.
  ...
  NOTE on `tosmonitor.registry/source-domain-matches-company?`: this
  company's own website (`www.tepco.co.jp`) and archived source-url
  (`www.tepco.co.jp/legal/`) are the SAME host, so the check's known
  two-part-TLD (`.co.jp`) base-domain limitation (see `tosmonitor.registry`
  ns docstring) does not affect this repo's own positive case -- it would
  only matter for distinguishing TWO DIFFERENT `.co.jp` companies from each
  other, which this repo's own governor contract tests do not exercise."
...
(defn demo-company []
  {:legal-name "東京電力ホールディングス株式会社"
   :website    "https://www.tepco.co.jp"})

(def ^:private demo-full-text
  "サイトのご利用にあたって\n本サイトのご利用にあたっては、以下の利用条件に同意いただいたものとみなします。当社は、本サイトの内容を予告なく変更することがあります。本規約の準拠法は日本法とします。")

(defn demo-baseline []
  {:full-text    demo-full-text
   :source-url   "https://www.tepco.co.jp/legal/"
   :retrieved-at "2026-07-19T07:41:07Z"
